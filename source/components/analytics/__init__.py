...
def make_limit_iters(struct, delay, parts_count):
...
async def analyze(figies, min_price_incs):
    ...
    parts_count = requests_count_constraint // requests_count_per_task
    ...
    offset, limit_iter = make_limit_iters(ticker_interval_tuples, delay, parts_count)
    for ind in limit_iter():
