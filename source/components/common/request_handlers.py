...
from serializers.tinkoff_invest_serializers import OrderbookResponse, StocksResponse
from serializers.tinkoff_invest.common import Stock
from source.app.config import config
from source.app.extentions import logger, session_provider
...
async def get_figi_and_min_price_inc(ticker: str):
    ...
    url = config.BASE_URL + '/market/search/by-ticker'
...
async def print_accounts():
    url = config.BASE_URL + 'user/accounts'
...
async def print_currencies_portfolio():
    url = config.BASE_URL + 'portfolio/currencies'
...
async def get_stocks() -> StocksResponse:
    url = config.BASE_URL + 'market/stocks'
...
async def get_orderbook(stock: Stock, depth: int) -> tuple[OrderbookResponse, Stock]:
    url = config.BASE_URL + 'market/orderbook'
